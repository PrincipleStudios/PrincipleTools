...
- To see if a candidate has the potential to grow, we share some of our
  (non-client) code and treat it like an onboarding session! Since an interview
  ...
  We also believe that code should be read dozens of times more often than it is
  written, so having candidates read code during the interview sets that trend
  early. After a canned bit of onboarding, we let them look anywhere in that
  codebase, answering any questions or highlighting some of the parts we're most
  excited about.
